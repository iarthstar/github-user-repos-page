...
let reposAPI = axios.create({
  baseURL: HOST + API + SERVICE['USERS'],
  headers: headers
});

reposAPI.interceptors.request.use(request => {
...
export const getReposAPI = (queries = {}, pagination = {}) => {
  const config = {};
  const username = queries.username;
  delete queries.username;

  config.params = { ...queries, ...pagination };
  return reposAPI.get(BACKEND_ENDPOINTS.repos(username), config);
};